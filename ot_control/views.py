...
def ot_booking_home_view(request):
	bookings = OTScheduleModel.objects.filter(date__range=[datetime.date.today(), datetime.date.today() + datetime.timedelta(days=7)]).order_by('date') # get all bookings within the next 7 days
...
def update_ot_bookng_view(request, pk):
	task = "Update Operation Theater Booking"
	booking = OTScheduleModel.objects.get(id=pk)

	# check if the user is the owner of the booking
	if request.user != booking.patient.user: # if the user is not the owner of the booking
...
def users_ot_booking_view(request, pk):
	...
	bookings = OTScheduleModel.objects.filter(patient=patient).order_by('date') # get all bookings for the patient
	context = {
		'bookings': bookings,
	}
